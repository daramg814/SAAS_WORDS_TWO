...
## 2c. 알고리즘 정밀 튜닝 라운드(2026-08-11) — "소량으로 다방면으로"

사용자 지시: 데이터원이 아니라 군집화 **알고리즘**을 다시 연구하되, 매번
대량 실측 대신 작은 합성 문장으로 저비용 검증하며 여러 각도로 시도하라.

방법과 결과 요약(상세는 `ACTIVE_ISSUES.md` `DEMAND-001` "여섯 번째 확인"):
실제 실패 군집의 문장 몇 개를 합성 케이스로 재현 → `clustering.
combined_similarity` 등을 직접 호출해 원인 진단 → 최소 수정 → 합성 케이스
+ 기존 테스트로 재확인 → 로컬 데이터로 저비용 재클러스터링 1회 교차검증
(새 네트워크 수집 없음). 이 방식으로 **진짜 버그 2개**를 찾음:
`clustering.STOPWORDS`(~40단어 임시 목록)에 "your"/"way"가 빠져 있었고,
이게 "Ask HN: How do you manage your X?"(X 다름)와 "way too complicated/
expensive"(무관한 대상)를 각각 12명·22명짜리 가짜 군집으로 묶는 원인이었다.
표준 크기 영어 기능어 목록으로 교체(커밋 `3e7a31b`). 추가로 GitHub 기본
이슈 템플릿 헤더 어휘를 `text_filter.GENERIC_COURTESY_TOKENS`에 3라운드
소량 추가. **실측 결과**: 고득점(5명 이상) 군집 34→25→20개(41% 감소),
최대 군집 크기 22→10명. 테스트 8개 추가, 회귀 없음(pytest 426 passed).

**정직한 한계**: 남은 20개 군집을 전부 다시 읽어도 여전히 100%
보일러플레이트(새로운 변형들)였다 — 이 방식은 수렴하되 완전히 0에
도달하지는 않을 가능성이 높다(보일러플레이트 종류가 사실상 무한). 그래도
토큰 비용이 낮아 **다음에 다시 써도 좋은 유효한 방법**으로 기록함.
