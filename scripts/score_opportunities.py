...
from saas_words_two import db, ids, opportunity_scoring, supply
...
def build_opportunity_row(conn, problem_row, demand_row) -> dict:
    ...
    grade = supply.scarcity_grade(effective_supply_value, direct_competitor_count, scarcity_result.total)

    priority = opportunity_scoring.priority_score(
        demand_row["total"], scarcity_result.total, effective_supply_value
    )
...
def persist_opportunity(conn, row: dict, updated_at: str) -> None:
    conn.execute(
        ...
        "evidence_ids=excluded.evidence_ids, product_ids=excluded.product_ids, updated_at=excluded.updated_at",
        (
            row["problem_id"],
            row["demand_score"],
    ...
    try:
        ...
        for problem_row in problems:
            ...
            if demand_row is None:
                continue
            opportunity_row = build_opportunity_row(conn, problem_row, demand_row)
