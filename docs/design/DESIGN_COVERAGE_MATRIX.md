...
## 2026-08-11 프로젝트 정의 전환에 따른 의도적 범위 축소

`CLAUDE.md` §1(현재 유효한 프로젝트 정의)에 따라 원본 설계서의 `# 7. 수요 예측
파이프라인`·`# 8. 공급 예측 파이프라인`·`# 9. 공급 희소성 우선 기회 판정`이
설명하는 실행 경로는 사용자 지시로 **보류**됐다(삭제 아님 — 코드는
`src/saas_words_two/pipeline.py`에 그대로 보존, 문서는 아래 표에 매핑된 파일에
그대로 보존). 현재 실행 경로(`word_pipeline.py`)는 이 세 절의 원문 예시(9절의
"대상+기능" 조합 구조)만 부분적으로 계승하고 나머지(수요 점수·공급 조사·기회
점수)는 적용하지 않는다. 이것은 원본 대비 발견하지 못해 누락된 항목이 아니라
CLAUDE.md §1에 명시적으로 문서화된 의도적 결정이다 — 매핑 수·미매핑 수(위 92/0)
는 "원본 절이 대응 문서에 보존되어 있는가"만 검증하며, "그 절의 로직이 현재
실행되는가"는 별도 질문이다(후자는 `memory/HANDOFF.md` §2b가 최신 상태를 유지).
