...
## Claude Code 실행 지침
1. 1차 후보는 max(target×1.6, target+20), 이후 부족분×2를 최대 5라운드 생성한다.
2. 최소 5개 기회에 분산하고 한 기회가 최종 결과의 30%를 초과하지 않게 한다.
3. 정확히 두 영단어, 단일 공백, Title Case를 하드 검증한다.
4. 정확·대소문자·역순·현재 실행·과거 이력·blocklist 중복을 코드로 제거한다.
5. 최대 라운드 후에도 부족하면 RETRYING 상태로 남기고 최종 파일과 운영 이력을 갱신하지 않는다.
