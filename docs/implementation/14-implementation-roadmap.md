...
**2026-08-19 개정(사용자 지시로 전체 문서 감사 후 발견 — 이 절이 아래 원본보다
우선한다):** 이 문서가 다른 canonical 문서들과 달리 두 차례 프로젝트 정의
전환(2026-08-11/2026-08-18) 반영 없이 옛 지침을 그대로 노출하고 있어 새
세션이 현재 지침으로 오인할 위험이 있었다 — 여기서 바로잡는다. 아래 원본
1~3번("1차 구현: HN→...→제목 500", "2차 구현: 데이터원 추가", "3차 개선:
군집 정확도·공급 누락·신뢰도 보정")은 **전부 폐기됐다** — 그 파이프라인
(HN 수집·군집화·수요/공급 점수·기회 산출) 자체가 2026-08-18에 코드·문서·
테스트까지 완전히 삭제됐고(git 이력에 보존), "정확히 500/20개" 목표·완료
개념도 같은 날 폐기됐다. 4번(설계서 경로 임의 변경 금지)은 지금도 유효하다
— 다만 그 "설계서"는 이제 CLAUDE.md와 `memory/KNOWLEDGE_MANIFEST.yaml`의
`canonical_documents`다.

**지금 유효한 구현 상태**는 단계별 로드맵이 아니라 반복 실행 모델이다:
`src/saas_words_two/word_pipeline.py`(진입점 `run.py`)가 유일한 파이프라인이고
"한 번의 CLI 실행 = 한 라운드"로 목표 수량 없이 계속 반복된다. 현재 워크플로우는
CLAUDE.md §6과 `docs/operations/11-workflow-state-memory.md`가 정식 기준이며,
이 문서는 더 이상 "다음에 뭘 구현해야 하는지"를 안내하지 않는다.
