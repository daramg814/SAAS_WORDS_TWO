...
**2026-08-18 프로젝트 정의 전환(CLAUDE.md §1과 동일한 결정, 현재 유효, 2차):**
수요·공급 관문(아래 원본 1~2번)은 `DEMAND-001`(일곱 차례 실측 실패)로
2026-08-11에 이미 보류됐었고, 이번 전환으로 그 파이프라인 자체가 코드·문서·
테스트까지 완전히 삭제됐다(git 이력에 보존). 동시에 "정확히 500/20개 확보"
목표·완료 개념과 업계 30% 분산 상한도 폐기됐다 — 프로젝트는 이제 업계별
단어뱅크(`src/saas_words_two/word_bank.py`)로 대량 원시 후보를 생성하고
Keyword Planner로 걸러 4개 문서(CLAUDE.md §4)를 목표 수량 없이 계속 누적하는
것만 목표로 한다. 아래 원본 1~2번과 500개/words.txt 관련 문구는 전부 폐기됐다고
읽는다. 3~4번(범위 제외, 품질 조건 불변)은 지금도 그대로 유효하다.

1. 실제 SaaS 제품 구현, 유료 데이터 구매, 무차별 크롤링, 법률 확정 의견은 범위 밖으로 유지한다.
2. 성공/실패 기준에 포함된 형식·중복·세션 복구·Git 조건을 임의 완화하지 않는다.
3. 단어뱅크의 각 단어는 실제 업계 전문용어·업무 개념에 근거해야 하며, 근거 없이 임의로 지어낸 단어를 섞지 않는다. 업계 커버리지 자체는 `src/saas_words_two/word_bank.py`의 `DOMAIN_WORDS`/`FUNCTION_WORDS`와 그 모듈 docstring이 유일한 근거다(코드가 곧 문서).
4. 조합된 제목은 여전히 "어떤 SaaS인지 추측 가능"해야 한다 — 이 판정의 책임은 전적으로 제목 검토 단계(현재 세션)에 있다.
